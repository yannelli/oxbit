...
- `bun run test:tasks` - config round trips, real process/services, and real Git worktree hooks.
- `bun run test:tasks:browser` - built browser UI against a real runtime (run `bun run build` first).
- `bun run desktop:test:build && bun run test:tasks:native` - native WebView/private storage workflow.
- `bun run remote:prepare && bun run remote:test` - isolated offline SSH fixture, including service port forwarding.
