...
describe('runtime streaming and symlink entries',()=>{
  ...
  it('bounds task replay and rejects invalid replay sequence numbers',async()=>{const root=await fs.mkdtemp(path.join(os.tmpdir(),'oxbit-task-stream-'));directories.push(root);const processes=new TaskRunner(await TaskConfigStore.create(root,{home:root}),()=>{});try{const task=await processes.run('owner',`${process.execPath} -e "process.stdout.write('x'.repeat(1200000))"`);const start=Date.now();while(processes.list('owner')[0]?.exitCode===undefined&&Date.now()-start<5000)await new Promise(resolve=>setTimeout(resolve,10));const replay=processes.attach(task.id,'owner');expect(replay.exitCode).toBe(0);expect(replay.truncated).toBe(true);expect(replay.chunks.reduce((size,chunk)=>size+Buffer.byteLength(chunk.data),0)).toBeLessThanOrEqual(1048576);expect(()=>processes.attach(task.id,'owner',-1)).toThrow('Invalid task sequence');}finally{await processes.close();}});
