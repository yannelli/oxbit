...
describe('runtime streaming and symlink entries',()=>{
  ...
  it('renames and deletes internal symlink entries without deleting their target',async()=>{const root=await fs.mkdtemp(path.join(os.tmpdir(),'zapp-links-'));directories.push(root);await fs.mkdir(path.join(root,'target'));await fs.writeFile(path.join(root,'target/file.txt'),'keep');await fs.symlink('target',path.join(root,'alias'));const files=new WorkspaceFiles(root);await files.rename('alias','renamed');expect((await fs.lstat(path.join(root,'renamed'))).isSymbolicLink()).toBe(true);await files.delete('renamed');expect(await fs.readFile(path.join(root,'target/file.txt'),'utf8')).toBe('keep');await fs.symlink(root,path.join(root,'target/back'));expect((await files.list('target')).map(entry=>entry.name)).not.toContain('back');});
});
