    ...
    pub fn link(self) {
        ...
        for package in self.packages {
            ...
            if xcode27 {
                ...
                globalize_runtime_symbols(&search_path.join(format!("lib{}.a", package.name)));
            }
...
/// The SwiftRs support module travels inside every package archive as its own
/// `SwiftRs.o` member, so `globalize_cdecl_symbols` skips it: that function only
/// touches the package's own member. Xcode 27 internalizes these three the same
/// way, and the `swift_rs` crate itself references them, so the final link fails
/// with "undefined symbols" for them alone. Promote them and rebuild the archive
/// index, which `llvm-objcopy` leaves stale.
fn globalize_runtime_symbols(archive: &std::path::Path) {
    const RUNTIME: [&str; 3] = ["_retain_object", "_release_object", "_string_from_bytes"];
    if !archive.exists() {
        return;
    }
    let Some(objcopy) = rustup_llvm_objcopy() else {
